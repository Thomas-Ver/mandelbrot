...
struct render{
    ...
    int iter;
    float ray;
    struct pixdiv *img; /* je me suis arreter la ou on definit le struc pixdiv*/
...
void image_save_grey(struct render *set, char *argv[]);  
...
void render_init(struct render *set, char *argv[]){
    ...
    set->L = strtol(argv[3],&endPtr,0);
    set->ray = 2;
    set->img = malloc(sizeof(struct pixdiv)*set->l*set->L); /*derniere modification ici aprés plus le temps*/
    if ( set->img == NULL ) {
...
void render_save_altern(struct render *set, char *argv[]){
    ...
    strcpy(set->basename,strcat(argv[4],".ppm"))   ; /*ici on concatene .pbm au nom de notre fichier pour créer le fichier dans le mode pbm*/
    fout=fopen(set->basename,"a");
    if ( fout == NULL ) {
            printf("Impossible d'ouvrir le fichier %s\n", set->basename) ;
            exit(1); 
...
void image_save_grey(struct render *set, char *argv[]){
    ...
    strcpy(set->basename,strcat(argv[4],".pgm"))   ; /*ici on concatene .pbm au nom de notre fichier pour créer le fichier dans le mode pbm*/
    fout=fopen(set->basename,"a");
    if ( fout == NULL ) {
            printf("Impossible d'ouvrir le fichier %s\n", set->basename) ;
            exit(1); 
    ...
    fprintf(fout,"P2\n%d %d\n256\n#nombre d'itération = %d\n#zone du plan complexe : x [%f,%f] , y [%f,%f]\n",set->l,set->L,set->iter,set->minx,set->maxx,set->miny,set->maxy); 
    s=0;
    for (i=0;i<set->L;i++){
        fprintf(fout,"\n");
        for (j=0;j<set->l;j++){
            fprintf(fout,"%d \n",(int)floor(set->img[s]*256.0/set->iter));
            s++;
